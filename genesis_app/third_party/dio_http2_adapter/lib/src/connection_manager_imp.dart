...
class _ConnectionManager implements ConnectionManager {
  ...
  void _throwIfH2NotSelected(Uri target, SecureSocket socket) {
    if (socket.selectedProtocol != 'h2') {
      ...
      // This socket has not entered the connection pool. The HTTP/1.1 adapter
      // opens its own connection, so closing the manager cannot clean it up.
      socket.destroy();
      throw DioH2NotSupportedException(target, protocol);
    }
  }
}
